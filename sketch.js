...
let dialogueState = 'idle'; // 對話狀態: 'idle', 'asking', 'answered'
...
function draw() {
  ...
  const proximityThreshold = 150; // 觸發問候的距離閾值
  ...
  if (isNewCharHit) {
    ...
    const textYOffset = 150; // 對話框在角色上方的高度
    ...
    if (dialogueState === 'asking') {
      // 將輸入框定位在角色1的頭上
      nameInput.position(characterX - nameInput.width / 2, characterY - 150);
    }
